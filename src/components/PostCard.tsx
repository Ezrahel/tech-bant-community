...
// ─── Pure helpers hoisted to module scope ────────────────────────────────────
...
// Category colour is currently uniform; kept as a function for future extension.
const CATEGORY_COLOR = "text-gray-300 bg-gray-800/50";
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center flex-wrap gap-1.5 sm:gap-2 mb-2 sm:mb-3">
            <div className="flex items-center space-x-1.5 sm:space-x-2">
              <span className="font-bold text-white text-xs sm:text-sm truncate max-w-[100px] sm:max-w-none">
                {post.author.name}
              </span>
            ...
            <span
              className={`text-[9px] sm:text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider ${CATEGORY_COLOR}`}
            >
              {post.category}
            </span>
          </div>
          ...
          <div className="flex items-center justify-between pt-3 sm:pt-4 border-t border-gray-800/50">
            ...
            <div className="flex items-center space-x-1 sm:space-x-2">
              <button
                ...
                disabled={loading || !isAuthenticated}
                title={
                  isAuthenticated ? "Bookmark post" : "Sign in to save posts"
                }
...
// memo: skips re-render when the parent list re-renders but `post` prop is
// unchanged (e.g. sibling optimistic-update on like/bookmark).
