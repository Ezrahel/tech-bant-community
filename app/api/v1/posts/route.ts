...
function isMissingPostsTableError(
...
function isMissingHtmlContentColumnError(
  error: { code?: string; message?: string } | null | undefined,
) {
  return (
    error?.code === "PGRST204" ||
    error?.message?.includes("html_content") === true
  );
}
  ...
  try {
    ...
    if (error) {
      if (isMissingPostsTableError(error)) {
        console.warn(
          "Falling back to sample posts because public.posts is missing from Supabase schema cache.",
        );
...
export async function POST(req: NextRequest) {
  try {
    ...
    const location = body.location
      ? sanitizePlainText(body.location, 100)
      : null;
    ...
    if (error && htmlContent && isMissingHtmlContentColumnError(error)) {
      console.warn(
        "posts.html_content column missing; retrying insert without rich text. Run supabase-migrations/003_posts_html_content.sql.",
      );
    ...
    if (error) {
      ...
      if (isMissingPostsTableError(error)) {
        return errorResponse(
          "Posts are not available yet. Please run the database migrations in Supabase.",
          503,
        );
      }
